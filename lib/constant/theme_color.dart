import 'package:flutter/material.dart';

class ColorConstant {
  static const Color success = Color(0XFF48B11B);
  static const Color secondaryColor = Color(0XFFFA8334);
  static const Color primaryColor = Color(0xFF0066ff);
  static const Color lightBackgroundColor = Color(0xFFe5f0ff);
  static const Color textPrimaryBlack = Color(0xFF292929);
  static const Color textPrimaryWhite = Color.fromARGB(255, 255, 255, 255);
  static const Color textBody = Color(0xFF6E8387);
  static const Color error = Color(0xffEF233C);
  static const Color borderGrey = Color(0xFFE0E4E6);
}
